...
const StyledContainer = styled.div<{
  ...
  hasError && tw`border-line-main`,
]);

const StyledInput = styled.input(({ inSize, disabled, label, hasError }: StyledInputProps) => [
  tw`box-content flex-1 h-4 min-w-0 px-4 py-5 leading-none placeholder-grayText-100 bg-transparent text-start text-14 text-gray-black text-ellipsis`,
  disabled && tw`text-gray-300 placeholder-grayText-100 opacity-100`,
  label && tw`px-4 pb-3 pt-7`,
  hasError && tw`placeholder-error-main`,
  inSize === 'small' && tw`[height: 32px] pl-4 pr-2 py-0 text-12 leading-4`,
  inSize === 'medium' && tw`h-4 px-4 py-4 leading-4 text-14`,
...
const StyledNumericInput = styled(({ inSize, label, hasError, ...others }: NumericFormatProps & StyledInputProps) => (
  ...
  label && tw`px-4 pb-3 pt-7`,
  hasError && tw`placeholder-error-main`,
  inSize === 'small' && tw`[height: 32px] px-4 py-2 text-12 leading-4`,
  inSize === 'medium' && tw`h-4 px-4 py-4 leading-4 text-14`,
  inSize === 'medium' && label && tw`px-4 pt-6 pb-2`,
]);
...
const StyledTextArea = styled.textarea(
  ({ inSize, disabled, hasError }: TextAreaProps & { inSize: SizeType; hasError: boolean }) => [
    tw`box-border flex-1 min-w-0 px-4 py-3 leading-8 placeholder-grayText-100 bg-transparent resize-none text-start text-14 text-gray-black`,
    inSize === 'medium' && tw`leading-6 text-12`,
    disabled && tw`text-gray-300 placeholder-grayText-100 opacity-100`,
    hasError && tw`placeholder-error-main`,
    css`
      &::-webkit-scrollbar {
...
const StyledLabel = styled.div<{ inSize: SizeType; focused: boolean; disabled: boolean; hasError: boolean }>(
  ({ inSize, focused, disabled, hasError }) => [
    tw`absolute top-0 left-0 leading-none text-gray-100 transition-all translate-x-4 translate-y-5 pointer-events-none text-14`,
    disabled && tw`text-gray-300 opacity-100`,
    focused && tw`leading-none translate-y-3 text-12`,
    hasError && tw`placeholder-error-main`,
    inSize === 'small' && tw`leading-none translate-x-4 translate-y-4 text-12`,
    inSize === 'medium' && tw`leading-none translate-x-4 translate-y-4 text-14`,
...
const Suffix = styled.span<{ inSize: SizeType; label?: string; disabled?: boolean }>(({ inSize, label, disabled }) => [
  tw`box-content h-4 px-4 py-5 leading-none text-14`,
  disabled && tw`text-gray-300 opacity-100`,
  label && tw`px-4 pb-3 pt-7`,
  inSize === 'small' && tw`[height: 32px] px-4 py-2 leading-4 text-12`,
  inSize === 'medium' && tw`h-4 px-4 py-4 leading-4 text-14`,
  inSize === 'medium' && label && tw`px-4 pt-6 pb-2`,
]);
...
const Border = styled.div<{ variant: VariantType; hasError: boolean; focused: boolean; disabled: boolean }>(
  ({ focused, variant, hasError, disabled }) => [
    ...
    variant === 'outlined' && tw`border border-main-default`,
    variant === 'outlined' && focused && tw`border-main-default`,
    variant === 'outlined' && disabled && tw`border-gray-100`,
    hasError && tw`border border-error-main`,
  ],
);
...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, value: valueProp, onChange, onFocus, onBlur, ...inProps }, ref) => {
...
const TextArea = forwardRef<HTMLTextAreaElement, Omit<TextAreaProps, 'as' | 'theme'>>(
  ({ value: valueProp, onChange, onFocus, onBlur, ...others }, inRef) => {
    ...
    useEffect(() => {
      if (typeof window === 'undefined' && innerRef.current) {
...
function ErrorMessage({ children }: ErrorMessageProps) {
  return (
    <div tw="flex mt-2">
      ...
      <span tw=" leading-4 pl-1 text-error-main">{children}</span>
    </div>
  );
}
...
function SuccessMessage({ children }: SuccessMessageProps) {
  return (
    <div tw="flex mt-2">
      ...
      <span tw="leading-4 pl-1 text-success-main">{children}</span>
    </div>
  );
}

const HelperMessage = tw.div`text-end  leading-3 mt-2 h-3 pl-1 `;
